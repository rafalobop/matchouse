...
- [x] **`tenantAuthMiddleware` no usa `getTenantClient()` (KAN-12, resuelto 2026-07-14):** el middleware ahora asigna `req.supabaseClient = getTenantClient(token)` en ambos branches (cache-hit y validación real), en vez del cliente service-role global. RLS ya aísla tenants en el flujo real de la app, no solo como defensa en profundidad. Validado con script de aislamiento en vivo contra Supabase (lecturas y escrituras cross-tenant bloqueadas por RLS; funcionalidad de service-role global intacta). Suite de tests: 80/80 en verde.
- [ ] **`web_push_subscriptions` no existe en el schema real (hallazgo 2026-07-02):** el código (`notifier.ts`) consulta esta tabla pero nunca fue creada — notificaciones web push están silenciosamente rotas en producción.
