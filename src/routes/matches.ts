...
// KAN-142: lectura y curación de matches del matching ciego (blind_matches), extraído de
// src/index.ts. No dispara matching en vivo (eso vive en routes/search.ts) — solo lee lo ya
// persistido y permite marcar el feedback del buscador (ACCEPTED/REJECTED).
...
router.get('/api/matches', tenantAuthMiddleware, async (req, res) => {
...
router.get('/api/matches/incoming', tenantAuthMiddleware, async (req, res) => {
  const tenantId = (req as any).tenantId;
  const supabase = (req as any).supabaseClient;
  try {
    const { data: dbMatches, error } = await supabase
...
router.post('/api/matches/:id/feedback', tenantAuthMiddleware, async (req, res) => {
  const tenantId = (req as any).tenantId;
  const { id } = req.params;
